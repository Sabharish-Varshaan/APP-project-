...
def data_analysis(request):
    ...
    weight_data = list(HealthMonitoring.objects.filter(user=request.user)
                   .annotate(truncated_date=TruncDate('date'))
                   .values('truncated_date')
                   .annotate(value=Avg('weight'))
                   .order_by('truncated_date'))
    ...
    weight_data = rename_key(weight_data, 'truncated_date', 'date')
    ...
    'weight_data': json.dumps(weight_data, cls=DjangoJSONEncoder),
